  ...
  const handleSave = () => {
    ...
    if (newVout !== currentVout) {
      ...
      if (nodes.some((n) => n.id !== props.id && n.data?.Vout === newVout)) {
        toast.error(
          `${newVout} já está em uso. O Vout do bloco não foi alterado.`
        );
        setForm((f) => ({ ...f, Vout: currentVout }));
        setOpen(false);
        return;
    ...
    if (newId !== currentId) {
      ...
      if (nodes.some((n) => n.id !== props.id && padId(n.data?.id) === newId)) {
        toast.error(
          `ID ${newId} já está em uso. O ID do bloco não foi alterado.`
        );
        setForm((f) => ({ ...f, id: currentId }));
        setOpen(false);
        return;
    ...
    setNodes((nodes) =>
      nodes.map((n) =>
        n.id === props.id
          ? { ...n, id: newId, data: { ...n.data, ...form, id: newId } }
          : n
      )
    );
    if (
      (idChanged || voutChanged) &&
      typeof updateConnectedVins === "function"
    ) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <div
          ref={nodeRef}
          className={`bg-transparent rounded w-[150px] h-[150px] border-2 border-transparent flex flex-col items-center justify-center ${
            isDarkMode ? "text-white" : "text-black"
          } font-bold relative cursor-pointer transition-all duration-200 ${selectionStyles}`}
        ...
        >
          <Handle
            ...
            className={`!absolute !-left-3 !w-3 !h-3 border-0 ${
          ...
          <Handle
            ...
            className={`!absolute !-right-3 !w-3 !h-3 border-0 ${
              isDarkMode ? "!bg-white" : "!bg-black"
            }`}
          />
        ...
        >
          {PARAMS.map((param) => (
            <div key={param.key} className="flex items-center gap-2">
              <label htmlFor={param.key} className="w-14 text-right text-xs">
                {param.label}:
              </label>
              <input
                id={param.key}
