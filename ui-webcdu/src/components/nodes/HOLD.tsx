...
export function HOLD(props: NodeProps & {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          <Handle id="vin2" type="target" position={Position.Bottom} className={`!absolute !-bottom-3 !w-3 !h-3 border-0 ${isDarkMode ? "!bg-white" : "!bg-black"}`}  />
          ...
          <svg className="w-[150px] h-[150px]">
            <line x1="75" y1="150" x2="75" y2="100" stroke={isDarkMode ? "#fff" : "#000"} strokeWidth="2" />

            <rect x="0" y="37.5" width={150} height={75} rx={10} ry={10} fill={isDarkMode ? "#333" : "#fff"} stroke={isDarkMode ? "#fff" : "#000"} strokeWidth="2" />
            
            {props.type === "thold" && (<text x="75" y="83" fontFamily="Arial" fontSize="25" fill={isDarkMode ? "#fff" : "#000"} textAnchor="middle">{"T/HOLD"}</text>)}
            {props.type === "shold" && (<text x="75" y="83" fontFamily="Arial" fontSize="25" fill={isDarkMode ? "#fff" : "#000"} textAnchor="middle">{"S/HOLD"}</text>)}
